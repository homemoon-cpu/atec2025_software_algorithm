...
class ExplorationState:
    NAVIGATING = "navigating"  # 沿拓扑边移动中
    SCANNING = "scanning"     # 在节点处进行全景扫描
    OPERATING_DOOR = "operating_door"  # 执行开门操作
    RESCUING = "rescuing"     # 执行救援动作
...
class agent:
    ...
    def __init__(self):
        ...
        self.current_node: Optional[TopologyNode] = None
        self.current_plan: List[int] = [] # 当前路径规划序列
    ...
    def get_scene_feature(self, image: np.ndarray) -> np.ndarray:
        """
        提取场景特征向量（CLIP视觉编码器）
        返回512维numpy数组
        """
    ...
    def get_semantic_description(self, image: np.ndarray) -> str:
        """
        生成场景语义描述
        返回示例："一个带有应急出口标识的走廊"
        """
        prompt = """请用中文简洁描述此场景，要求：
        1. 不超过20个字
        2. 包含主要结构和显著物体
        3. 直接输出描述文本，无需额外格式"""
    ...
    def _exploration_based_on_topology(self):
        # 优先选择拓扑图中未探索的相邻节点
        next_node_id = self.planner.plan_next_move(self.current_node, self.topology)
        ...
        # **判断目标类型是否为门**
        target_node = self.topology.nodes[next_node_id]
        if 'door' in target_node.semantic.lower():
            self._set_target_as_door(target_node)
            return self._transition_to_door_move()
        
        # 普通地标移动
        self._set_target_landmark(target_node)
        return self._transition_to_landmark_move()
    
    def _update_topology(self):
        """核心拓扑更新逻辑"""
        # 提取当前场景语义和特征
        semantic = self.get_semantic_description()
        feature = self.get_scene_feature()
        landmarks = self.detect_landmarks()
        
        # 回环检测
        matched_id = self.topology.match_node(feature)
        if matched_id is not None:
            self.current_node = self.topology.nodes[matched_id]
            self.current_node.landmarks.extend(landmarks)  # 更新地标信息
            return
        
        # 发现新节点
        new_node = self.topology.add_node(semantic, feature)
        ...
        if self.current_node:
            self.topology.add_connection(
                ...
                new_node.id,
                cost=1.0  # 可根据实际移动难度调整
            )
        
        self.current_node = new_node

    def _handle_navigation(self) -> Tuple[list, float, float]:
        """拓扑导航状态处理"""
        if not self.current_plan:
            # 生成新的探索路径
            self.current_plan = self._plan_exploration_path()
        ...
        # 获取下一个目标节点
        next_node_id = self.current_plan[0]
        next_node = self.topology.nodes[next_node_id]
        
        # 执行移动动作
        action = self._navigate_to(next_node)
        
        # 到达目标节点后更新状态
        if self._check_arrival(next_node):
            self.current_plan.pop(0)
            self.current_node = next_node
            self.state = ExplorationState.SCANNING  # 到达后执行扫描
            
        return action
    
    def _plan_exploration_path(self) -> List[int]:
        """生成最优探索路径（A*算法变种）"""
        def cost_heuristic(node_id):
            node = self.topology.nodes[node_id]
            # 优先选择：未探索边 > 含门节点 > 近邻节点
            unexplored_edges = sum(
                1 for conn in node.connections.values() if not conn["explored"]
            )
            door_priority = 10 if "door" in node.semantic else 0
            return 100*unexplored_edges + door_priority
        
        # 查找全局最优目标节点
        target_node_id = self.topology.find_optimal_target(cost_heuristic)
        
        # 使用A*算法生成路径
        return self.topology.a_star_path(
            self.current_node.id, 
            target_node_id,
            heuristic=cost_heuristic
        )
    
    def _handle_scanning(self) -> Tuple[list, float, float]:
        """节点全景扫描处理"""
        # 执行三视角扫描动作
        ...
        # 扫描完成后更新节点状态
        if action == ([0, 0], 0, 0):
            self.current_node.fully_explored = True
            self.state = ExplorationState.NAVIGATING
        
        return action
    
    def _handle_door_operation(self) -> Tuple[list, float, float]:
        """门操作处理"""
        # 检测门状态
        if self._is_door_open():
            # 发现新区域，更新拓扑连接
            new_nodes = self._detect_beyond_door()
        ...
        # 执行开门动作序列
        return self._execute_door_opening_sequence()

    def _handle_search_phase(self):
        if self.info['picked'] == 1:
            self.phase = 3

        """处理输入图像，返回控制指令(线速度, 角速度, 持续时间)"""
        # 1. 更新拓扑地图
        self._update_topology(self.obs)
        
        # 2. 状态机决策
        if self.state == ExplorationState.NAVIGATING:
            return self._handle_navigation()
        elif self.state == ExplorationState.SCANNING:
            return self._handle_scanning()
        elif self.state == ExplorationState.OPERATING_DOOR:
            return self._handle_door_operation()
        elif self.state == ExplorationState.RESCUING:
            self.phase = 2
            return self._handle_rescue_phase()
            ...
            try:
                ...
                if match:
                    ...
                    if door_direction == "TOO_CLOSE":
                        ...
                        if self.need_to_verify_indoor:
                            self.action_buffer.append(([0, 50], 0, 1))  # 向前移动通过门
                            self.obs_buffer.append(False)
